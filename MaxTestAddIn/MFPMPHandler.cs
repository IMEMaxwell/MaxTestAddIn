...
namespace MaxTestAddIn
{
    ...
    public class MFPMPHandler : IPropertyManagerPage2Handler9
    {
        ISldWorks iSwApp;
        SwAddin userAddin;
        MFPMP ppage;

        // variables for face and face persistent ID
        byte[] facePID;

        public MFPMPHandler(SwAddin addin, MFPMP page)
        {
            userAddin = addin;
        ...
        public void OnClose(int reason)
        {
            ...
            // function to handle the PMP if pressed ok
            if(reason == (int)swPropertyManagerPageCloseReasons_e.swPropertyManagerPageClose_Okay)
            {
                ...
                byte[] facePIDByte = facePID;
                string[] mparamNames = { "FacePID" };
                int[] mparamTypes = { (int)swMacroFeatureParamType_e.swMacroFeatureParamTypeString };
                string[] mparamValues = { GetStringFromID(iSwApp, swModel, facePIDByte) };

                object paramNames = mparamNames;
                object paramTypes = mparamTypes;
                object paramValues = mparamValues;

                Feature swMacroFeature = swFeatMgr.InsertMacroFeature3("Macro Feature", "MaxTestAddIn.MacroFeature",
                    null, mparamNames, mparamTypes, mparamValues,
                    null, null, null, null,
                    (int)swMacroFeatureOptions_e.swMacroFeatureByDefault);

                if (swMacroFeature == null)
                {
        ...
        {
            if (id == MFPMP.buttonID1)        // Toggle the textbox control visibility state
            {
            ...
            else if (id == MFPMP.buttonID2)   // Toggle the textbox control enabled/disabled
            {
        ...
        public void OnSelectionboxListChanged(int id, int item)
        {
            ...
            // if selection box list changed
            if(id == MFPMP.selection1ID)
            {
                // if more than 1 item selected
                if (item > 0)
                {
                    ModelDoc2 swModel = iSwApp.ActiveDoc;
                    ...
                    // get selected face
                    object swFace = swSelMgr.GetSelectedObject6(1, -1);

                    // get the persistent ID and cast to byte[]
                    facePID = (byte[])swModelExt.GetPersistReference3(swFace);
                }
            }
        }
        ...
        // function to convert persistent ID to string
        // copy from https://help.solidworks.com/2023/English/api/sldworksapi/Use_Persistent_Reference_Example_CSharp.htm
        public string GetStringFromID(ISldWorks swApp, ModelDoc2 swModel, byte[] vPIDarr)
        {
